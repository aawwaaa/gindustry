...
func _on_config_button_pressed() -> void:
    var node = await Vars.mods.mod_inst_list[mod_info.id]._open_configs();
...
func load_info(info: ModInfo) -> void:
    ...
    %ConfigButton.disabled = not Vars.mods.mod_inst_list.has(mod_info.id)
    ...
    for depend in mod_info.depends.keys():
        ...
        if Vars.mods.mod_info_list.has(depend):
            line.get_node("ModName").text = Vars.mods.mod_info_list[depend].name
        else:
            line.get_node("ModName").text =  depend+" "+tr("Vars.mods_Unsloved")
        line.get_node("ModVersion").text = versions[0] + " ~ " + versions[1] \
            if versions.size() > 1 else versions[0] + " +"
        line.get_node("Button").disabled = not Vars.mods.mod_info_list.has(depend);
        line.pressed.connect(load_info);
        %ModDepends.add_child(line);
        if Vars.mods.mod_info_list.has(depend):
            line.mod_info = Vars.mods.mod_info_list[depend]
    ...
    for except in mod_info.excepts:
        var line = mod_line.instantiate();
        line.get_node("ModName").text = Vars.mods.mod_info_list[except] \
            if Vars.mods.mod_info_list.has(except) \
            else except+" "+tr("Vars.mods_Unsloved")
        line.get_node("ModVersion").text = "*"
        line.get_node("Button").disabled = not Vars.mods.mod_info_list.has(except);
        line.pressed.connect(load_info);
