...
func load_ui(node: Control) -> void:
    ...
    GameUI.instance.build_ui.toggle_build_mode_buttons(true)
...
func unload_ui(node: Control) -> void:
    GameUI.instance.build_ui.toggle_build_mode_buttons(false)
...
func handle_drag(event: InputEvent, world_pos: Vector2) -> bool:
    ...
    if Input.is_action_pressed("build_break_drag") or Input.is_action_just_released("build_break_drag"):
        ui.build_mode = "break"
    elif Input.is_action_pressed("build_copy_drag") or Input.is_action_just_released("build_copy_drag"):
        ui.build_mode = "copy"
    else:
        ui.build_mode = "place"
...
func handle_place_drag(event: InputEventMouse, world_pos: Vector2) -> bool:
    ...
    if Input.is_action_just_released("build_build_drag"):
        dragging = false
        confirm_build_drag()
    return true
...
func handle_break_drag(event: InputEventMouse, world_pos: Vector2) -> bool:
    ...
    while current_pos.y <= end.y:
        ...
        current_pos.y += 1
    if Input.is_action_just_released("build_break_drag"):
        dragging = false
